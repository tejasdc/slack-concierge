...
- **Pebble Index 01** → transcript-only multipart webhook at `https://capture.tejas.nyc/pebble` → isolated durable `capture_events` queue → trusted Concierge delivery → configured Slack destination (`#slack-inbox`) → router agent. The phone performs speech-to-text; server Whisper is not on this latency-sensitive path.
- **Slack messages / DMs** → bot writes into `vault/inbox.md`.

External HTTP capture routes live in `config/capture-routes.toml`. A route selects its path, adapter, body limit, systemd credential, and destination; the ingress implementation contains none of those flow bindings. Caddy owns origin TLS and proxies to the loopback-only `agent-inbox.service`; a narrow Cloudflare Worker gives Pebble the readable hostname. The capture service runs as `concierge-capture` inside a strict filesystem sandbox and receives no Slack or provider credential. It owns the queue and exposes an authenticated second loopback listener to the trusted Concierge service, which delivers with its existing Slack user token. Owner-bound capture leases and a durable failure-hold gate prevent deploys from racing delivery or resuming it before both services pass functional health. The old `/audio` behavior and the Pebble route remain configured adapter/destination pairs behind the same public-ingress boundary.
