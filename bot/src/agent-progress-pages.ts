...
export function progressBlocks(chunks: ProgressChunk[]): Record<string, unknown>[] {
  const isPlan = (chunk: ProgressChunk) => chunk.type === "task_update" && chunk.id === "plan-progress";
  return [...chunks.filter(c => !isPlan(c)), ...chunks.filter(isPlan)].flatMap((chunk): Record<string, unknown>[] => {
    if (chunk.type === "plan_update" || chunk.type === "steering_boundary") return [];
...
export function splitRejectedProgressPage(chunks: ProgressChunk[]): ProgressChunk[][] {
  ...
  if (textIndex >= 0) {
    ...
    return parts.map((part, index) => {
      ...
      const page: ProgressChunk[] = [...boundaries, ...before, { type: "markdown_text", text: part }, ...(index === parts.length - 1 ? content.slice(textIndex + 1) : [])];
