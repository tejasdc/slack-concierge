...
export function canonicalAgentsTemplate(projectName: string, codePath: string) {
  return [
    ...
    "- Keep each agent-authored to-do to one concise, high-level action or pointer. Put design, requirements, research, and work logs in `docs/`, then link the row to that durable document with a bare URL when available. Multi-paragraph capture remains a transport format, not an agent work log. Slack `Rec…` comments are transient projection metadata, never durable item identity.",
    "",
