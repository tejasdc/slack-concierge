...
export function attachmentPrompt(
  ...
  return [
    ...
    "Continue the one session that owns this surface when search/context establish its exact address, dialogue and send capability; a topical match or consultation-only source is insufficient. If no session owns this work or the surface is different, create a fresh session with --provider cx-sol --effort medium --project <exact-project-folder> --session-name <title>. Use sessions projects and match the actual code project; Concierge code belongs to slack-concierge and Thinkering code to thinkering. D0BMWUJ3RD5 is not a fallback. Clarify ambiguous ownership or a missing project rather than guessing. Honor an explicit human session/provider/model/effort choice. Do not route through deprecated Slack posts. Preserve original bytes and use retained capture custody if these temporary files have expired.",
  ].join("\n");
}
