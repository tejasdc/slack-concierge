...
export function attachmentPrompt(
  ...
  return [
    ...
    "For ordinary new work, create a fresh session with --provider cx-sol --effort medium --project <exact-project-folder> --session-name <title>, even if related older sessions exist. Use sessions projects and match the actual code project; Concierge code belongs to slack-concierge and Thinkering code to thinkering. D0BMWUJ3RD5 is not a fallback. Ask if the target is missing or unclear. Honor an explicit human request to resume a specific session or select another provider/model/effort; establish an intended existing address with sessions search/context. Do not route through deprecated Slack posts. Preserve original bytes and use retained capture custody if these temporary files have expired.",
  ].join("\n");
}
