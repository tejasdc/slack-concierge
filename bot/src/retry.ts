...
/** Durable queue callers persist `attempt` and `startedAtMs`, then ask this same policy for the next due time. */
...
export async function withRetry<T>(input: Retryable<T>): Promise<T> {
  ...
  const exhausted = (attempts: number, reason: RetryBudgetExhaustedError["reason"]): never => {
    const error = new RetryBudgetExhaustedError(input.operation, attempts, Math.max(0, now() - startedAtMs), lastError, reason);
    ...
    publishRetryBreakerNotice({ key: `${input.operation}:${input.key}:${startedAtMs}`,
      what: input.operation, reason: String(lastError), sinceMs: startedAtMs,
