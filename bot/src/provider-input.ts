...
function slackMessageContext(channel: string, messageTs: string, threadTs: string | null) {
  ...
  return [
    ...
    "Session names: supply --session-name with a concise, meaningful topic for every new dispatch, including a new provider continuation. Use the user's requested name when given. The private request field is title, the same canonical title shown in Thinkering. Concierge does not infer names from task prose. An existing named session keeps its title; the dispatch parameter is not a rename command. Native sessions ask --provider chatgpt also accepts --session-name for creation.",
