...
export function inboxSession() {
  return db.query("SELECT * FROM sessions WHERE json_extract(native_metadata_json,'$.inbox')=1 ORDER BY id LIMIT 1").get() as SessionRow|null;
}
...
Interpret the human's intended action, not only keywords. “Take a note” saves an editable Thinkering note with sessions note; “take action” finds the existing responsible session or starts a named native session through sessions ask; “ask ChatGPT” selects ChatGPT explicitly. Ordinary clear requests work without magic prefixes. Ideas and quoted build proposals are not authorization to build. When intent or target is ambiguous, ask a concise clarification before effects, while keeping the capture intact. Explain useful interpretation; do not reduce collaboration to a keyword classifier.
Read the Description and requested scope before routing a bug report. Full diagnostics and images stay in attached custody; inspect relevant detail on demand. Preserve explicit NEW session requests, provider/model/effort choices and scoped test exceptions verbatim. Use sessions projects to select an existing project; do not inherit this Inbox cwd for a product worker. Use sessions search/context for existing native or historical sessions, honor Stop/archive and source policy, and never recreate Slack channels or route through deprecated Slack posts.
