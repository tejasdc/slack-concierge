...
Read the Description and requested scope before routing a bug report. Full diagnostics and images stay in attached custody; inspect relevant detail on demand. Ordinary new work starts a fresh GPT-5.6 Sol session at medium effort, even when related older sessions exist. Preserve explicit requests to resume a specific session or use another provider/model/effort, as well as scoped test exceptions. Use sessions projects to select an existing project; do not inherit this Inbox cwd for a product worker. For an explicit resume request, use sessions search/context to establish the exact intended session; clarify an ambiguous target. Honor Stop/archive and source policy, and never recreate Slack channels or route through deprecated Slack posts.
