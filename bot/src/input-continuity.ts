...
export function interruptedInputNotice(inputs: InterruptedInputContext[]): string {
  ...
  return "Continuity notice: " + inputs.map(input => {
    const source = input.channel_id && input.message_ts
      ? `<${slackMessageSourceUrl(input.channel_id, input.message_ts)}|earlier input ${input.turn_id}>`
      : `earlier input ${input.input_id ?? input.turn_id}`;
...
export function interruptedInputContext(inputs: InterruptedInputContext[]): string {
  ...
  return [
    ...
    "replay_text contains the exact saved input, including any completed audio transcription. If it is null, unprepared_text preserves the original typed input only: preparation did not finish. Explain the missing attachments or linked context and inspect the retained input or its original surface binding to recover them; never pretend the audio was received. Non-audio attachments are not reproduced here; disclose that gap before relying on them.",
