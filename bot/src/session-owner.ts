...
export class SessionOwner {
  ...
  create(body:unknown) {
    ...
    const saved=db.transaction(()=>{
      ...
      const session=createNativeSession(input.provider,{title,purpose:input.purpose,workflowId:input.workflowId,cwd:project?.cwd??this.defaultCwd,...(project?{project:project.cwd}:{}),...defaults});
      this.validateAttachments(session,input.firstInput?.attachments);
