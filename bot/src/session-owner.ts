...
import {parseProviderSelector,normalizeReasoningEffort,resolveProviderSelector,PROVIDER_ALIASES} from './aliases';
...
export class SessionOwner {
  ...
  create(body:unknown) {
    ...
    const saved=db.transaction(()=>{
      ...
      const session=createNativeSession(input.provider,{title,purpose:input.purpose,workflowId:input.workflowId,cwd:project?.cwd??this.defaultCwd,...(project?{project:project.cwd}:{}),...(input.model?{model:input.model}:{}),...(input.reasoningEffort?{reasoningEffort:input.reasoningEffort}:{})});
      this.validateAttachments(session,input.firstInput?.attachments);
