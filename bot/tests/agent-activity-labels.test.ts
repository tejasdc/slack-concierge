...
describe("native activity labels", () => {
  test("uses Codex command classification rather than its shell wrapper", () => {
    for (const [action, title] of [
      [{ type: "read", name: "AGENTS.md", path: "/project/AGENTS.md" }, "Reading AGENTS.md"],
      [{ type: "listFiles", path: "/project/src" }, "Listing src"],
      [{ type: "search", path: "/project/src", query: "private search" }, "Searching src"],
    ] as const) {
  ...
  test("includes multiple classified actions without raw commands or queries", () => {
    ...
    expect(activity).toMatchObject({ title: "Inspecting files", details: "Reading a.ts\nSearching files" });
