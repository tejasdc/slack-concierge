...
export async function runThinkeringSlackCase(options: {
  ...
  const receipts: ThinkeringReceipt[] = [];
  const submit = async (request: ThinkeringRequest, authorized = true) => {
    const receipt = await options.adapter.submitThinkeringCapture(request, authorized);
  ...
  for (const long of [false, true]) {
    ...
    if ((await submit(request, false)).http_status !== 401) throw new Error("Unauthenticated capture accepted");
    ...
    const duplicate = await submit(request);
    ...
    if ((await submit({ ...request, text: "changed" })).http_status !== 409) throw new Error("Changed content reused a capture identity");
  ...
  for (let index = 0; index < requests.length; index++) {
    const receipt = await submit(requests[index]!);
  ...
  const result = { case_id: "thinkering-slack", lane_id: options.lane.lane_id, run_id: options.runId,
    marker, observations, receipts, run_owned_unsettled: 0, status: "passed" };
