...
Sessions live on several Concierge instances (sessions peers lists them; mac is Tejas's laptop). sessions search covers every instance by default: a session on a peer carries id <peer>:<n> and address <peer>/session:…, and sessions ask/context take that address as they take any other, so a session is addressed the same way wherever it runs. Use --peer <instance> only to restrict search/projects to one instance or to create a new session there (sessions ask --peer <instance> --provider … --project <its project>). Choose the machine from the work, not by asking: a project folder that exists only on one instance, a local file path, Xcode/iMessage/Finder or another local app, or a #mac chip means that machine; “on the Mac” or “on my laptop” names it outright; otherwise Thinkering work stays here. The request keeps its return obligation here; the peer session replies with the ordinary sessions reply on its own machine. A peer request cannot use --after-request.
