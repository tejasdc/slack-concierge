...
Explicit --provider creates one new native session and first input, with an exact request/operation and automatic correlated result. Provider aliases and effort use the existing alias table; use the configured cx default unless the human explicitly requests another model or effort. Codex/Claude require --project from sessions projects; the owner resolves its cwd. ChatGPT accepts no project or effort. No provider fallback or Slack publication occurs.
