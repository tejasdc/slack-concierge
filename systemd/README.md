...
`bot/scripts/service-notice.ts --key <episode> --title <title> -- <text>` is the provider-free notice entry point for host jobs. It opens the existing ledger with `CONCIERGE_STATE_DIR`, prints `recorded` for a new Inbox reading notice or `duplicate` for the same key, and exits nonzero on failure. The failed-service unit uses systemd's `MONITOR_INVOCATION_ID` ([systemd.exec](https://www.freedesktop.org/software/systemd/man/latest/systemd.exec.html)) in its key so one failed episode creates one notice; it falls back to the unit's last invocation ID if that variable is unavailable. The app itself uses the same Inbox writer when a retry breaker trips.

`post`, `resume`, and `upload` submit the task and exact dependencies through the running service's private `requests.sock`. The service owns durable publication and normal input admission, uses the existing user token, and returns the request status with its exact Slack receipt. Only `status: admitted` confirms admission. `work` provides execution lookup and durable request recovery. `audit` retains the bot-token transport and confirms the triggering message's root before posting. `thread-of`, `resolve-upload`, and `permalink` remain bounded read-only receipt tools. See the [router helper runbook](../docs/runbooks/ROUTER-ACTIONS.md) for syntax, failure handling, and caller migration.
...
| `concierge-service-failure@.service` | Publishes one provider-free Inbox notice when Concierge or capture ingress exhausts five restarts in ten minutes | [waiting and retrying](../docs/plans/2026-09-24-waiting-and-retrying.md) |
...
Router historical discovery dispatches global or channel-filtered `threads search`,
exact-candidate `threads context`, and `threads stats` to
