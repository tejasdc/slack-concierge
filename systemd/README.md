...
| `concierge-deployment-provider-adapter.service` | Root-only, capability-gated adapter in a private runtime namespace that injects the existing Codex credential without exposing it to workers or kernel role sockets | [deployment repair architecture](../docs/architecture/DEPLOYMENT-REPAIR.md) |
