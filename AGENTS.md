...
- Treat `notes/TODOS.md` as the canonical todo document. Slack Lists is a read-only outbound projection with stable row IDs, never an input surface, prompt context, or an agent-output control protocol. File changes are watched outside the interactive turn queue and List API calls use an isolated rate-limit lane.
...
| Deployment batching, provenance, post-deploy wake turns, and failure notices | `bot/src/deployment-state.ts`, `bot/src/deployment-worker.ts`, `bot/scripts/deploy-state.ts`, provider environment plumbing, and `bot/tests/deployment-state.test.ts` |
