...
Slack does not expose installed slash commands in a thread reply composer, so `/fork` is a channel-composer convenience and the `Fork from here` message shortcut is the thread-native point-in-time surface. A successful fork always creates a new top-level Slack anchor; replying to that anchor continues the child provider session. An explicit session already bound to a visible Slack thread always outranks channel-wide `single-persistent` routing, so fork and comparison children cannot fall back to the channel's default provider session. Never post the fork anchor as a reply in the source thread, because Slack has no nested threads and the child session would have no routable visible thread root.

Codex turns persist their App Server turn ID on the owning Concierge turn. `Fork from here` passes that exact ID as `lastTurnId` to App Server `thread/fork`, inclusive. Every message shortcut and explicit `/fork <message-ts>` requires a proven turn boundary; selecting a fork anchor or unrelated Slack message can never fall back to cloning the latest hidden session. An original request whose provider turn later accepted steering is also unrepresentable because the Codex turn ID includes that later guidance; fork its completed agent response instead. For a legacy turn, Concierge reads the source Codex thread and backfills the boundary only when that turn's canonical replay text uniquely identifies one Codex turn; zero or multiple matches are explicitly rejected instead of silently cloning the wrong hidden history. Codex forks set `deferGoalContinuation=true`, so cloning a session with an active goal cannot start automatic work before the user sends a request in the new Slack thread. Claude Code exposes whole-session forking but no proven point-in-time boundary, so its message shortcut is rejected while bare `/fork` remains available for the latest complete session.

Fork creation is a durable `fork_requests` lifecycle keyed by Slack's trigger ID. The row persists a unique Codex `threadSource` recovery marker and Slack `client_msg_id` before provider execution, moves through `claimed → forking → forked → delivering → binding → delivered`, and records the child provider UUID before attempting Slack delivery. `binding` durably records Slack's returned top-level timestamp before local session creation; Slack is never reposted after reaching that state. Ingress treats any channel-local `delivering` request, plus `forked` retry state after a prior Slack attempt, as a short admission barrier until that timestamp is known; replies to the exact root then wait through binding. Finalization refuses every provider-agnostic visible-root collision instead of allowing a different provider to win routing. Transient Slack failures before a timestamp return to `forked` and retry with the same client message ID without dropping that barrier; permanent failures park without forgetting the child. Startup and periodic reconciliation prove the prior process owner dead before reclaiming work. An interrupted Codex request scans App Server threads using the `vscode` source kind emitted by the top-level Codex 0.147 `app-server` command, then locally requires both the exact `forkedFromId` and recovery marker to recover an acknowledged child; `parentThreadId` is a sub-agent filter and must not be used for fork recovery. Zero matches may retry only from a dead `forking` owner, multiple matches remain ambiguous, and Claude interruptions remain ambiguous because its CLI has no equivalent lookup key. Only invalid-parameter JSON-RPC responses are treated as definite fork rejection; internal errors remain ambiguous and enter marker recovery. Creating the top-level `sessions` row and marking the fork request delivered are one SQLite transaction.
