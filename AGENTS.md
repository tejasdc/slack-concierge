...
- Keep the deployment-repair control plane fail-closed during staged rollout. The protected root kernel is the only control-DB and release-effect writer; role-specific sockets fix caller identity; the non-root coordinator and bot submit typed requests only. `CONCIERGE_DEPLOYMENT_CONTROL_ENABLED`, `CONCIERGE_ENABLE_CONTROL_REQUESTS`, and `CONCIERGE_AUTONOMOUS_REPAIR_ENABLED` remain disabled until the documented provider-isolation, non-root candidate, restore, notifier, real-unit security-negative, and live rollback gates all pass. `deploy.sh` remains the operator fallback until that cutover is verified.
