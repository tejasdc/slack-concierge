...
- Never keep a provider turn alive to wait for, monitor, or verify a deployment that it requested. After the deployment request is durably handed to the detached supervisor, end the current turn and let the deployment workflow own drain, activation, health proof, notification, and exact-session wake. Do not create or retain a persistent goal, polling loop, background waiter, scheduled wake, or repeated status check whose completion condition is that deployment: every resumed provider turn is active work and can indefinitely prevent the zero-active-turn drain. Run follow-up verification only through the persisted post-deploy wake or in a later user-initiated turn after the deployment notification.
