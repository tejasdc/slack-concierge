...
- Keep provider execution behind the project-scoped broker boundary once containment is enabled. The bot may reach only each managed project's broker socket and shared attachment scratch; it never receives provider credentials or the worker socket. The broker derives executable, home, project/additional roots, environment, model, sandbox, approval policy, and session binding from root-owned project authority. Socket templates and pinned provider bundles may be installed inertly, but only the journaled application cutover may create and enable project instances.
