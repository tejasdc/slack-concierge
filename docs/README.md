...
- [Thinkering captures](runbooks/THINKERING-CAPTURE.md) — notes and bug reports through the authenticated capture contract, frozen content, receipts, session-identity boundaries, credentials, and safe sandbox integration.
...
- [Older MacBook session consultation handoff](plans/2026-09-15-mac-session-consultation-handoff.md) — completed experiments, verified native archive-search implementation owner, committed Mac read/search capability, hard consultation-only policy at discovery/runtime, and the ChatGPT browser-path amendment. Consultation acceptance remains open.
