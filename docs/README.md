...
- [Thinkering Send to Slack](runbooks/THINKERING-CAPTURE.md) — authenticated capture contract, stable snapshot IDs, full-content delivery, receipts, credentials, and safe sandbox integration.
...
- [Native Agent progress messages](plans/2026-08-26-agent-progress-messages.md) — historical implementation plan for the native in-place progress transport; current bounded history, steering-only successors, planning, and Stop behavior are documented in [the turn lifecycle architecture](architecture/TURN-LIFECYCLE.md).
...
- [Agent-session discovery, identity, and communication ownership](brainstorms/2026-09-14-agent-session-discovery-ownership.md) — design-only composition contract for native-first Slack-independent session keys, a Thinkering-owned catalog, source-grounded search/context, imported-session capabilities, lineage, and session-addressed communication across runtime adapters.
- [Readwise and QMD search evaluation](brainstorms/2026-09-14-readwise-qmd-search-evaluation.md) — research-only comparison of hosted Reader retrieval and local QMD, including current Markdown coverage/freshness gaps, measured operating costs, and the evaluation required before any augmentation.
