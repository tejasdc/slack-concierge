...
- **Linked session title** is an exact canonical workspace permalink with the root `thread_ts`; there is no separate navigation link beneath the row. In Slack Web this route is intercepted by the existing client: selecting the bold title replaces the main pane with the thread, the Home dashboard remains in split view, and no browser tab is created. The authenticated workspace URL comes from `auth.test`, with `team.info` as a startup fallback; only if Slack supplies neither does the title fall back to the generic client URL.
