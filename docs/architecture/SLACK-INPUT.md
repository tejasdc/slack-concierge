...
- Claude sends `control_request`/`interrupt` before the replacement user message. Both the successful control response and the exact echoed user event produced by `--replay-user-messages` prove acceptance; Concierge does not require optional `isReplay` metadata that the CLI contract does not promise. An aborted result is intermediate; the stream remains open for the replacement result.
