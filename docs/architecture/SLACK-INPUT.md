...
- Claude writes the follow-up as a uuid-stamped async user message into the CLI's own streaming-input command queue. It does not interrupt: the message runs at the agent's next step or after the current response, on the CLI's schedule. The `--replay-user-messages` echo carrying that uuid proves acceptance; exact text equality is kept only as a fallback for a CLI that does not return the client uuid. No per-message deadline applies while the turn is live, because a queued message is legitimately unacknowledged until the CLI dequeues it — possibly after a long tool call. A pipe write the process refused is provably unsent. Stop is the only interrupt the adapter sends.
