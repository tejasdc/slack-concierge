...
Historical resume discovery uses the credential-free `threads search` helper. Concierge supplies its exact-channel/cutoff and clarify-on-unresolved contract beside each input identity; attachments preserve the same resume decision before forwarding. Corpus, transactional projection ownership, completeness, and current-session resolution are documented in [router search](ROUTER-SEARCH.md).
