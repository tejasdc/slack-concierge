...
Authority: `bot/src/canvas.ts`, callers in `bot/src/index.ts` and `bot/src/turn-execution.ts`, scaffold ownership in `bot/src/channel.ts`, and focused Canvas and turn-lifecycle tests in `bot/tests/canvas.test.ts` and `bot/tests/turn-execution.test.ts`.
