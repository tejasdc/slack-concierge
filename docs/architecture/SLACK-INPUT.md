...
Every real Slack input sent to a provider starts with a `<slack-message-context>` JSON block containing `channel_id`, `message_ts`, and `thread_ts` as strings. The event supplies the channel and exact triggering message timestamp; its `thread_ts`, or its own timestamp for a root, supplies the reply target. `prepareProviderInput` carries that identity alongside text/transcripts into initial, resumed, queued, and steering input. Queue reconstruction uses the persisted input's reply root, never the single-persistent session anchor. Each steering message gets its own block; the initial turn's identity is not reused. The block is stored with canonical replay text so reconstructed provider history retains the same input identity. Missing or malformed real-message timestamps fail preparation instead of guessing. Synthetic comparison/deployment input explicitly has no triggering-message block.

Router audit calls use the `channel_id` and `message_ts` from the block belonging to the input being handled. No channel-history query, newest-message inference, or environment lookup is required. Historical blocks describe their historical inputs, not a new audit target; if no current triggering identity is supplied, the router must not fabricate one.
