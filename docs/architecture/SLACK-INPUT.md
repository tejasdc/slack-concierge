...
The router obtains its original triggering input through `trigger <turn-id>`, using the exact ID from the turn-owned artifact directory already supplied to it. The helper reads that running real-user turn's `slack_user_msg_ts` and `slack_reply_thread_ts`, joining its session for the channel only. Missing, stale, synthetic, or incomplete identities fail explicitly; no ambient turn ID, session anchor, or newest-message fallback is used. This is a local read through the existing helper, not additional provider-prompt plumbing. The [router helper runbook](../runbooks/ROUTER-ACTIONS.md) defines its output and caller usage.
