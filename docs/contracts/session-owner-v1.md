...
| `POST /sessions/v1/sessions` | Trusted authenticated human surface: `{clientActionId, provider, purpose, title?, workflowId?, project?, model?, reasoningEffort?, firstInput?}`. Provider is codex/claude-code/chatgpt; purpose is chat/extract/transform/develop. Coding models must be supported by the selected provider and effort must be a shared effort token. Development requires an exact project from the owner inventory; other coding sessions may select one. ChatGPT accepts no project, model, or effort. Returns canonical session and operation receipt. Optional firstInput is accepted atomically with creation. Explicit provider intent is retained; unavailable ChatGPT never silently becomes another provider. Without an explicit selection, the owner records its configured default model and effort at creation. |
