...
| `POST /sessions/v1/sessions/:id/message-actions` | `{clientActionId, action}` where action is `{kind:"reaction",messageId,emoji,present}` or `{kind:"save",messageId,present}`. The owner accepts only an exact retained message under that canonical session. `present:false` removes the exact emoji or saved mark. |
| `GET /sessions/v1/saved` | `{sessions: SessionView[], messages: [{session: SessionView,messageId,savedAt}]}`. Every saved message retains its exact canonical session and provider message ID so a consumer can reopen that message without substituting a nearby row. |
