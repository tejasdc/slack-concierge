...
All illustrated receipt keys are present; absent identities and unavailable facts are JSON `null`. `kind` is create/input/action/stop/reconcile/cancel/fork/consultation/request/reply; `origin` is human/agent/service and is assigned by the owner. `text` is the immutable accepted input/question/reply text, or null for a control. Timestamps are UTC ISO8601 strings with milliseconds. `request` is null or immutable accepted request metadata: human input may retain its delivery/expectedRunId, attachment IDs and application references; agent requests retain `{sourceInputId, sourceRunId, targetSessionId, requestedEffect, afterRequestIds}`. Dependencies are exact request IDs. These are the already-validated public request values, never private provider preparation, cwd or authority. `childSessionId` is null until a fork/consultation child is durably created, then its canonical ID. It never names the immutable imported source. An input's `runId` can be populated while queued; only `admission` proves provider admission. `admission` is defined below. `result` is retained native output text or null; it is not proof that any request was answered or workspace effect published. Errors are null or `{code, message}` with a stable code and safe explanatory text. Execution result, settlement and return evidence remain owner facts; surface adapters must preserve fields they do not render rather than translate them into a second operation ledger.
...
| `POST /sessions/v1/sessions/:id/inputs` | Trusted surface admission of a new input: `{clientActionId, text, attachments?, evidence?, selection?, intent?, procedure?, promptRevision?, workflowId?, delivery?, expectedRunId?}`. Server-issued input identity derives from authenticated ingress, not Slack or a preexisting source ID. |
...
| Create, actions, Stop, reconcile | `{session: SessionView, operation: Receipt}` |
...
The trusted surface validates selection/intent/procedure/prompt/workflow metadata against retained workspace references before constructing an input. Selection is an array of `{objectId,revision}`; intent and promptRevision use one such reference or null. Other application metadata retains its native typed representation; it does not start the deferred extraction/transform callbacks. An attachment upload is authenticated owner custody, not an executable operation. Receipt ID/hash/name/type are stable on an identical action; conflicting bytes/metadata return409. The accepted input pins authorized custody IDs before preparation; provider handoff verifies decoded bytes against those pins. Session artifact responses are `{name,contentType,base64,sha256}` and require an artifactId returned by that session's history/result. Native detail/artifact reads survive completed runs and do not replay work.
...
- `kind:"message"`, payload `{message:Message}` with exact stable message ID, common run alias on the envelope, native turn/submission/tool/detail/rich-content fields retained. The surface adapter can emit `{kind:"run",run}` or `{kind:"message",message}` after provider spelling conversion. A terminal run event remains terminal when a delayed transient progress event arrives.
...
`Message` retains the native history fields `{id, role, content, tool, phase}` (tool/phase nullable), and optional `detailKey`, `submissionId`, `toolCallId`, `turnId`, `attachments`, `richContent`, `source`. Message/native-turn IDs retain provider identity; `submissionId`, when present for owned ChatGPT input, is the common run UUID used as its provider effect ID. History attachment descriptors are `{id,name,contentType}`. Rich content preserves the existing version1 `parts` union (markdown/writing/file/unsupported); a file's native path is evidence, not permission to read a host path. The compatibility examples include stable source and native message identities.
...
| `bind` | `{sessionId, bindingGeneration, reference:{accountScope, sessionId, anchor}}` | `{binding: Binding}` after exact account/conversation/branch/event/hash verification |
...
For an already-started effect, the same immutable identity and pins select its existing native receipt. `observe`, `reconcile`, and an identical repeated start can return retained evidence even after the owner run is terminal, stopped or no longer active; they do not require state running and cannot send again. An uncertain/dispatched effect remains uncertain unless the existing native receipt proves acknowledgement/result. Read-only bind/history/snapshot/artifact likewise do not require an active run: they validate the canonical session's current binding generation and retained binding/source anchor under trusted owner authority. Bind returns evidence for the owner to adopt, never updates the canonical row itself. The owner resolves public artifactId to the exact retained native message/path; the capability rejects a path absent from that message. It never treats `path` as an arbitrary host filesystem path.
...
The JSON files in [session-owner-v1/](session-owner-v1/) are synthetic consumer/producer examples, not captured user data or a second API definition. Each has `{version:1,cases:[{name,request:{method,path,body?},response:{status,body},...}]}`. Wrapper and field types are normative; example IDs, opaque addresses, timestamps, titles and counts are sample values. Cases with `ownerReceipt`, `ownerSession`, `existingProviderReceipt` or `expect` supply test preconditions/assertions, never additional HTTP request fields. Consumers execute their own adapter against these exchanges and reject mismatched pins. Successful fixture parsing alone is not runtime or live acceptance.
