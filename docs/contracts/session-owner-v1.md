...
Actionable native setup/provider failures and uncertainty advance attention once for the exact accepted input/dispatch attempt, including unavailable creation before a turn exists. Results and failure projection share the durable attention marker; repeated observation, restart or recovery never increments it again. `kind:"attention"` events identify that accepted input and optional run and carry `{dispatchAttempt}`. They permit the existing surface to refresh its session view without implying provider completion or outcome change. Read/dismiss generations and outcome remain independent; startup catches retained failures without admitting or replaying work.
