...
| `GET /sessions/v1/auth/providers` | Trusted authenticated human surface: `{providers: ProviderAuthView[]}`, one per `claude-code`/`codex`. `mode` is `interactive` (a code is pasted back) or `device` (a code is typed into the browser and the host process completes on its own). `account` is the account whose credentials are on disk, or null; its `label` is a display name and never a token. `profiles` are named credential snapshots on this host. |
| `POST /sessions/v1/auth/refresh` | Trusted authenticated human surface: `{provider}` starts a login. Returns `awaiting_code` with `url`, or `awaiting_approval` with `url` and `userCode`, or a settled result. A device login has no completion call; observe `pending` on the providers read. |
| `POST /sessions/v1/auth/refresh/complete` | Trusted authenticated human surface: `{provider, code}` supplies a pasted approval code. Valid only for an `interactive` login awaiting one; otherwise `no_pending_login`. |
| `POST /sessions/v1/auth/profiles/save` | Trusted authenticated human surface: `{provider, label}` snapshots the credentials currently on disk under a chosen name. Returns `{profiles}`. |
| `POST /sessions/v1/auth/profiles/switch` | Trusted authenticated human surface: `{provider, profileId}` puts a saved account's credentials in place and activates them as one operation. |
...
| `turnOutcome` | `{outcome:"done"\|"needs_you"\|"failed"\|"finished_without_saying", question, inputId, at}` for the latest declared or recorded turn outcome, or null. |
...
Results, native setup/provider failures and uncertainty advance unread activity once for the exact accepted input/dispatch attempt, including unavailable creation before a turn exists. The retained `kind:"attention"` event name identifies this activity marker and carries `{dispatchAttempt}`; it does not by itself set `needsAttention`. Every working turn's final answer is structured output the provider validates: Claude Code `--json-schema`, Codex `outputSchema` on `turn/start`, both carrying `{outcome:"done"|"needs_you"|"failed", question?, message}` with `question` required for `needs_you` (`bot/src/turn-structured-output.ts`). `message` is the turn's text everywhere — history, Inbox results, returns — and the raw object is kept only as result evidence. A final hand-off reply's work disposition also declares (completed→done, failed→failed, needs_decision→needs_you, an informational final answer→done); the turn's own structured answer supersedes it. ChatGPT's answer counts as done; consultations declare nothing. A native turn that ends without a valid structured answer is recorded `finished_without_saying`, never inferred from text. `needsAttention` comes only from declared `needs_you` outcomes: each records a `kind:"turn_outcome"` event and a `kind:"needs_you"` attention event `{question,inputId,generation}`, where `inputId` is the input it answers — in the Inbox, the request thread's root. An open question clears when Tejas sends a message in that session (in the Inbox, a reply in that thread), when a later turn declares (in the Inbox, for the same thread), or when he dismisses it past its generation. Reading never clears it, and `finished_without_saying` neither raises nor clears it. `SessionView.turnOutcome` is `{outcome, question, inputId, at}` for the latest declaration, or null. `@Tejas` text no longer affects attention. Read/dismiss generations and outcome remain independent; startup catches retained failures without admitting or replaying work.
...
Explicit returned replies resolve the retained event's source input/run and responder; partial/final is retained separately. Automatic answered outcomes resolve their exact output turn; canceled, failed, unanswered and overdue notices are service-authored even if they carry a responder field. Proven communication requests/replies display their exact retained prose and custody attachments, while immutable provider wrappers remain available in raw execution evidence. Ledger-only fallback and Inbox history use their direct accepted-input relation, with no browser-side receipt inference.
