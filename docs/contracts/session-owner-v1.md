...
| `POST /sessions/v1/sessions/:id/message-actions` | `{clientActionId, action}` where action is `{kind:"reaction",messageId,emoji,present}` or `{kind:"save",messageId,present}`. The owner accepts only an exact message that session's own history shows: a live message event, an Inbox request, result or thread post, or a provider transcript message. `present:false` removes the exact emoji or saved mark. |
