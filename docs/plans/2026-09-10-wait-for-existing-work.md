...
Status: approved for implementation by Tejas's “ship it” on 2026-09-10. The implementation follows this whole-change design; current ownership is documented in [routed requests](../architecture/ROUTED-REQUESTS.md). The router never independently posts or attaches dependencies afterward. This supersedes the bot-authored proposal in `ff3e331` and the message-trigger-first framing in `fff0da5`.
...
Use focused regressions, exact-source Slack sandbox evidence, the repository's one fresh-context whole-diff review, corrections, and the final local gate. Deterministic tests force transport interruption and event orderings; the real Slack case proves publication, ownership, waiting, activation, attachments, and downstream dispatch.

## Evidence limits

Implementation uses the existing SQLite state, session queue, Slack transport, and reaction worker. `routed-requests.test.ts` exercises fixed references, destination FIFO, terminal outcomes, duplicate/early/missing echoes, receipt recovery, dependency chains, and stale reaction completion. The executable `queued-requests` sandbox case captures the exact runtime source and Slack identities with each run. Its controlled provider isolates lifecycle behavior from model reasoning.

Source inspection used targeted sections and verification searches; large files were not read in full and no LSP tool was exposed. The delivery record reports the actual test and sandbox outcomes; the current contract belongs to [routed requests](../architecture/ROUTED-REQUESTS.md).
