...
Status: approved for implementation by Tejas, relayed in input 1789455269.499469, with the removal-test and reversible-rollback corrections incorporated below. Concierge edits this one document; the Thinkering native owner's incorporation amendment is input 1789454508.918469 (attachment F0C1F24EF1V). The independently approved hop-cap removal, overdue-notice tests and documentation repair continue separately.
...
Activation must quiesce old Thinkering dispatch, recheck exact duplicate claims and live ownership, reconcile ambiguous effects, take consistent backups, import exact mappings once, then switch consumers. No dual writes or concurrent native owners. Preserve old stores and native histories as evidence.

Rollback must be executable and tested after the new owner has accepted work: quiesce the new owner, inventory its accepted/acknowledged/settled and unresolved effects, reconcile native outcomes, map new session/input/result records back into the restored owner's compatible store, restore the previous runtime, and prove native resume plus correlated delivery under one owner. Never overwrite newer native history with the pre-cutover backup or replay completed provider/application effects. Test clean rollback and rollback with an accepted request and pending return. If a particular state cannot be represented by the prior software, name that exact state and the concrete schema/provider limitation, preserve it for reconciliation, and confine the limitation to it. A blanket claim that any accepted work makes rollback impossible is prohibited.
...
- Rollback after new acceptance is exercised through quiescence, effect reconciliation, restored data/runtime, native resume and correlated return. No concurrent owner, lost accepted work or duplicated effect is permitted; any unrepresentable state is identified precisely rather than declared globally irreversible.
